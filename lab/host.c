...
static volatile uint16_t g_released_block = 0;
...
static void reset_recv_block_state(recv_vertex_state_t *rv, uint16_t block_id) {
    if (!rv) return;
    memset(rv, 0, sizeof(*rv));
    rv->active = 1;
    rv->vertex_id = (uint32_t)g_rank;
    rv->block_id = block_id;
    rv->neighbor_mask = neighbor_mask_of((uint32_t)g_rank);
}
...
static void send_block_done_to_router(uint16_t block_id) {
    uint8_t frame[HDR_LEN];
    int len = build_frame_ex(frame, g_my_ip, 0,
                             (uint32_t)g_rank, 0,
                             block_id, 0, 0,
                             1, 1, 0, NULL, 0);
    host_inject(frame, len);
...
static int block_all_received(const uint8_t *recvd, uint16_t block_id, uint32_t total_npkts) {
    uint32_t start = block_start_seq(block_id);
    uint32_t end = block_end_seq(block_id, total_npkts);
...
int m_send(int conn, const void *buf, uint32_t size, uint8_t op) {
    ...
    uint16_t total_blocks = total_blocks_for_npkts(total_npkts);
    ...
    for (uint16_t block_id = 0; block_id < total_blocks; block_id++) {
        uint32_t start = block_start_seq(block_id);
        uint32_t end = block_end_seq(block_id, total_npkts);
        uint32_t block_npkts = end - start;
        uint8_t *acked = calloc(block_npkts, sizeof(uint8_t));
        uint64_t *sent_at = calloc(block_npkts, sizeof(uint64_t));
        uint32_t base = 0;
        ...
        while (g_released_block < block_id)
            usleep(200);

        memset(sv, 0, sizeof(*sv));
        sv->active = 1;
        sv->buf = (const uint8_t *)buf + start * PAYLOAD_LEN;
        sv->seq_base = start;
        sv->npkts = block_npkts;
        sv->op = op;
        sv->seq_acked = acked;
        sv->neighbor_ack_bitmap = 0;
        sv->neighbor_mask = peer_mask;
        sv->vertex_id = (uint32_t)g_rank;
        sv->block_id = block_id;
        sv->ecn_pending = 0;
        sv->gap_us = 100;

        while (base < block_npkts || sv->neighbor_ack_bitmap != sv->neighbor_mask) {
            uint64_t now = now_us();
            if (sv->ecn_pending) {
            ...
            while (base < block_npkts && acked[base]) base++;
            uint32_t end_idx = base + WINDOW;
            if (end_idx > block_npkts) end_idx = block_npkts;
            for (uint32_t s = base; s < end_idx; s++) {
                uint64_t send_now = now_us();
                uint32_t seq = start + s;
                ...
                if (!acked[s] && (sent_at[s] == 0 || send_now - sent_at[s] >= RTO_US)) {
                    ...
                    conn_send_ex(cn, seq, 0, op, 0, resend_flag,
                                 (const uint8_t *)buf + seq * PAYLOAD_LEN, PAYLOAD_LEN);
                    sent_at[s] = send_now;
                    last_tx_at = send_now;
                }
            }
            if (base < block_npkts) usleep(200);
        }
...
int m_recv(int conn, void *buf, uint32_t size, uint8_t op) {
    ...
    uint8_t *recvd = calloc(total_npkts, sizeof(uint8_t));
    uint32_t got = 0;
    ...
    uint16_t active_block = 0;
    int block_done_sent = 0;
    int words_per_pkt = PAYLOAD_LEN / (int)sizeof(int32_t);
    if (op == OP_ALLREDUCE) {
        fetch_started = calloc(total_npkts, sizeof(uint8_t));
        pull_mask = calloc(total_npkts, sizeof(uint32_t));
        fetch_sent_at = calloc(total_npkts, sizeof(uint64_t));
        neighbor_cached_pkts = calloc(MAX_GROUP_SIZE, sizeof(uint32_t));
        local_sum = calloc(total_npkts * words_per_pkt, sizeof(int32_t));
        feature_cached = calloc((size_t)MAX_GROUP_SIZE * total_npkts, sizeof(uint8_t));
        feature_cache = calloc((size_t)MAX_GROUP_SIZE * total_npkts * words_per_pkt, sizeof(int32_t));
        remote_mask = neighbor_mask_of((uint32_t)g_rank);
        rv = recv_state_of((uint32_t)g_rank);
        reset_recv_block_state(rv, 0);
    }
    ...
    while (1) {
        ...
        for (int ci = 0; ci < g_conn_count; ci++) {
            ...
            while (conn_pop(&g_conns[ci], &m)) {
                ...
                if (m.is_ack == 1 && m.is_fetch == 1 && src_cn->remote_ip == 0) {
                    if (m.block_id > g_released_block)
                        g_released_block = m.block_id;
                    continue;
                }
                if (m.ecn)
                ...
                if (m.payload_len == 0 || m.seq_num >= total_npkts) continue;
                ...
                if (is_complete_result) {
                    send_vertex_state_t *sv = send_state_of(m.dst_id);
                    if (sv && sv->active && m.block_id == sv->block_id &&
                        m.seq_num >= sv->seq_base && m.seq_num < sv->seq_base + sv->npkts) {
                        sv->seq_acked[m.seq_num - sv->seq_base] = 1;
                    }
                }

                if (is_non_aggregated_feature) {
                    size_t cache_slot = (size_t)m.src_id * total_npkts + m.seq_num;
                    if (m.src_id < MAX_GROUP_SIZE && !feature_cached[cache_slot]) {
                        ...
                        if (block_id_of_seq(m.seq_num) == active_block) {
                            neighbor_cached_pkts[m.src_id]++;
                            if (rv && neighbor_cached_pkts[m.src_id] == block_end_seq(active_block, total_npkts) - block_start_seq(active_block) && m.src_id < 32)
        ...
        if (op == OP_ALLREDUCE && active_block + 1 < total_blocks && block_done_sent && g_released_block >= active_block + 1) {
            active_block++;
            block_done_sent = 0;
            memset(neighbor_cached_pkts, 0, sizeof(uint32_t) * MAX_GROUP_SIZE);
            reset_recv_block_state(rv, active_block);
            last_progress = now_us();
        }

        uint64_t now = now_us();
        if (op == OP_ALLREDUCE && !block_done_sent && active_block < total_blocks && rv && !rv->switch_result_seen &&
            now - last_progress >= RTO_US && now - last_fetch_at >= RTO_US) {
            uint32_t start = block_start_seq(active_block);
            uint32_t end = block_end_seq(active_block, total_npkts);
            ...
            for (int r = 0; r < g_n; r++) {
                ...
                if (!peer) continue;
                for (uint32_t s = start; s < end; s++) {
                    size_t cache_slot = (size_t)r * total_npkts + s;
                    if (recvd[s]) continue;
        ...
        if (op == OP_ALLREDUCE && !block_done_sent && active_block < total_blocks && block_all_received(recvd, active_block, total_npkts)) {
            rv->neighbor_recv_bitmap = rv->neighbor_mask;
            send_ack_to_neighbors((uint32_t)g_rank, active_block, op);
            send_block_done_to_router(active_block);
            block_done_sent = 1;
        }
        if (got >= total_npkts) {
